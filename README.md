...
与实物对照时，标准配色（Western/BOY）魔方按**黄顶、绿前**拿放——此时红面自然在左、
橙面在右（白底、蓝后）。打乱公式、录入与输出均以此拿法为准。
...
- **求解录入的魔方**：按录入说明填六面参数（可点"填入复原态示例"再改贴纸）→ 执行 → 输出分阶段公式
- **打乱演示**：点"生成随机打乱公式"→ 回车，从复原态打乱后直接求解（公式单独一行、长输入框展示）
